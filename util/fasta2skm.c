...
static gdl_string * PROGRAM = "fasta2skm";
...
// yunan
static gdl_string * PATTERNS = NULL;
...
parse_argument (int argc, char *argv[])
{
	...
	while (1)
	{
		...
		c = getopt_long (argc, argv, "i:o:d:k:t:p:",
		...
		// yunan
		case 'p':
			PATTERNS = gdl_string_clone (optarg);
			break;
		case '?':
...
check_argument (void)
{
	...
	if (PATTERNS == 0)
	{
		GDL_ERROR_VAL ("No patterns file provided", GDL_FAILURE, 1);
	}
	return GDL_SUCCESS;
}
...
main (int argc, char *argv[])
{
	...
	if (status == GDL_SUCCESS)
	{
		...
		// read LDPC patterns		
		//gdl_string * PATTERNS = gdl_string_clone("patterns.txt");
		gdl_string * line = 0;
		FILE * patterns = 0;
		size_t n_write, numHash, row_weight;
		size_t ** LDPC;

		patterns = gdl_fileopen (PATTERNS, "r");
		gdl_getline (&line, &n_write, patterns);
		toks = gdl_string_split(line, " ", &ntok);
		numHash = atoi(toks[0]);
		row_weight = atoi(toks[1]);

		if (kmer_sizes[0] % row_weight != 0)
		{
			GDL_ERROR_VAL ("K should be multiple of row_weight", GDL_FAILURE, 1);
		}

		LDPC = GDL_CALLOC(size_t*, numHash);
		for (i = 0; i < numHash; i++)
			LDPC[i] = GDL_CALLOC(size_t, row_weight);

		i = 0;
		while (gdl_getline (&line, &n_write, patterns)!=-1)
		{
			toks = gdl_string_split(line, " ", &ntok);
			for (j = 0; j < ntok; j++)
				LDPC[i][j] = atoi(toks[j]);
			i = i + 1;
		}
		for(i = 0; i < numHash; i++)
			for(j = 0; j < row_weight; j++)
				fscanf(patterns, "%zu", &LDPC[i][j]);
		gdl_fileclose (PATTERNS, patterns);
		/*
		for (i = 0; i < numHash; i++)
		{
			for (j = 0; j < row_weight; j++)
		...
		*/
		/*
		printf("%s\n", "before opened!");
		gzFile patterns = fopen("pattern.txt", "r");				
		size_t numHash, row_weight;
		size_t ** LDPC;		
		printf("%d\n", patterns);
		fscanf(patterns, "%zu %zu", &numHash, &row_weight);		
		printf("%zu %zu\n", numHash, row_weight);
		LDPC = GDL_CALLOC(size_t*, numHash);		
		for (i = 0; i < numHash; i++)
			LDPC[i] = GDL_CALLOC(size_t, row_weight);
			...
			for(l = 0; l < n_kmer_sizes; l++){
				...
				if(seq->seq.l >= kmer_size){
					for(i = 0; i <= seq->seq.l - kmer_size; i++){
						...
						// generate spaced kmer by LDPC patterns
						gdl_string *premer;
						premer = 0;
						premer = gdl_string_alloc (kmer_size);
						strncpy (premer, seq->seq.s+i, kmer_size);

						for(row = 0; row < numHash; row++){
							gdl_string *kmer;
							kmer = 0;
							kmer = gdl_string_alloc (row_weight);
							for(j = 0; j < row_weight; j++)
								kmer[j] = premer[LDPC[row][j]];
