...
#def split_fasta2(filename, dest_dir, num_splits, my_env):
#        prefix = os.path.basename(filename)
#        size = os.path.getsize(filename)
#        maxSize = int(math.ceil(size / (num_splits - 0.25)))
#        cmd = ["fastaSplit",filename,dest_dir,prefix,str(maxSize)]
#	#print("infasta: "+my_env["PATH"])
#	#print(sys.path)
#        num_counts = subprocess.check_output(cmd, env=my_env);
#        return num_counts
...
def split_fasta2(filename, dest_dir, num_splits, my_env):
        ...
        cmd = ["fastaSplit",filename,dest_dir,prefix,str(maxSize)]
        num_counts = subprocess.check_output(cmd, env=my_env);
        if int(num_counts) > num_splits:
                names = glob.glob(dest_dir+'/*/*.fasta')
                lfiles = names[-2:]
                out_dir = os.path.dirname(lfiles[0])
                out_dir_mod = out_dir+'1'
                safe_makedirs(out_dir_mod)
                merge_files2(lfiles, out_dir_mod,os.path.basename(lfiles[0]), "fasta", my_env)
                for l in lfiles:
                        x = os.path.dirname(l)
                        shutil.rmtree(x)
                os.rename(out_dir_mod,out_dir)
                num_counts = str(int(num_counts)-1)
        return num_counts
...
def merge_files2(filelist, out_dir, infile_name, ext, my_env):
        prefix = infile_name.rsplit('.',1)[0];
        cmd = ["fileMerge", out_dir,  prefix,  ext] + filelist
        outfile = subprocess.check_output(cmd, env=my_env)
        return outfile
...
if __name__ == "__main__":
	...
	dest_dir = args.o[0]
	num_splits = int(args.n[0])
	#print filename
	script_loc = os.path.realpath(__file__)
	sys.path.append(os.path.dirname(script_loc))
	my_env = os.environ.copy()
	my_env["PATH"]=(os.path.dirname(script_loc) + ":" + my_env.get("PATH", ""))
	my_env["LD_LIBRARY_PATH"]=(
        os.path.join(os.path.dirname(script_loc),'ext','gdl-1.1','GDL','lib') + ":" +
        my_env.get("LD_LIBRARY_PATH", ""))
	num_counts = split_fasta2(filename, dest_dir, num_splits, my_env)
	print(num_counts)
